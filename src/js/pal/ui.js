...
utils.extend(ui, {
   ...
   * @return {Box}
   */
  createBox: function(pos, rows, cols, style, saveScreen) {
    var borderRLE = [],
        sprite = ui.sprite;
    // Get the bitmaps
    for (var i = 0; i < 3; i++) {
   ...
   * @return {Box}
   */
  createSingleLineBox: function(pos, len, saveScreen) {
   ...
   */
  drawNumber: function(num, len, pos, color, align) {
   ...
   * @return {Promise}
   */
  readMenu: function*(onchange, list, defaultItem, labelColor, nocancel) {
...
/*
  ...
  getObjectDesc: function(list, id) {
    for (var i=0; i<list.length; ++i) {
      var it = list[i];
